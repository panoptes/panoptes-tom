...
GLOBAL_TARGET_FIELDS = ["field_name", "type"]
...
class Target(models.Model):
    ...
    name = models.CharField(
        ...
        default="",
        verbose_name="Name",
        help_text="The name of this target e.g. Barnard's star.",
        unique=True,
    )
    ...
    pm_ra = models.FloatField(
        null=True,
        blank=True,
        verbose_name="Proper Motion (RA)",
        help_text="Proper Motion: RA. Milliarsec/year.",
    )
    pm_dec = models.FloatField(
        null=True,
        blank=True,
        verbose_name="Proper Motion (Declination)",
        help_text="Proper Motion: Dec. Milliarsec/year.",
    )
    galactic_lng = models.FloatField(
        null=True,
        blank=True,
        verbose_name="Galactic Longitude",
        help_text="Galactic Longitude in degrees.",
    )
    galactic_lat = models.FloatField(
        null=True,
        blank=True,
        verbose_name="Galactic Latitude",
        help_text="Galactic Latitude in degrees.",
    )
    ...
    scheme = models.CharField(
        max_length=50,
        choices=TARGET_SCHEMES,
        verbose_name="Orbital Element Scheme",
        default="",
        blank=True,
    )
    ...
    lng_asc_node = models.FloatField(
        null=True,
        blank=True,
        verbose_name="Longitude of Ascending Node",
        help_text="Longitude of Ascending Node. J2000. Degrees.",
    )
    inclination = models.FloatField(
        null=True,
        blank=True,
        verbose_name="Inclination to the ecliptic",
        ...
        if created:
            for extra_field in settings.EXTRA_FIELDS:
                if extra_field.get("default") is not None:
                    TargetExtra(
                        target=self, key=extra_field["name"], value=extra_field.get("default")
                    ).save()
    ...
    def future_observations(self):
        ...
        return [
            obs
            for obs in self.observationrecord_set.all().order_by("scheduled_start")
            if not obs.terminal
        ]
    ...
    def extra_fields(self):
        ...
        return {
            te.key: te.typed_value(types[te.key])
...
class TargetList(models.Model):
    ...
    created = models.DateTimeField(
        auto_now_add=True,
        help_text="The time which this target list was created in the TOM database.",
    )
    modified = models.DateTimeField(
        auto_now=True,
        verbose_name="Last Modified",
